...
def ai_chatbot_page():
    st.title("AI Chatbot for Portfolio")
    st.info("Ask questions about the analysis you just completed.")

    # Ensure analysis data exists in session state
    if "gathered_data" not in st.session_state:
        st.error("No analysis data found. Please run the Group Analysis page first.")
        return
    
    analysis_data = st.session_state["gathered_data"]
    st.write(f"Chatbot is now trained with data")
    ...
    if user_input:
        ...
        chat_context = [
            {"role": "system", "content": "You are an AI assistant analyzing group vs group data based on gathered insights."},
